...
function populateInfo(i)
{
  ...
  if (i.u) {
	...
	${inforow("上线时间",getRuntimeStr(i.uptime))}
	${inforow("空闲堆",heap," kB")}
  	${inforow("电流估算",pwru)}
  	${inforow("帧率",i.leds.fps)}
	...
	for (var y = 0; y < (s.seg||[]).length; y++)
	{
		...
		cn += `<div class="seg">
			...
			<div class="segin ${expanded[i] ? "expanded":""}" id="seg${i}">
				...
				<table class="infot">
					<tr>
						<td class="segtd">起始 LED</td>
						<td class="segtd">终止 LED</td>
						<td class="segtd">偏移</td>
					</tr>
					<tr>
				...
				<table class="infot">
					<tr>
						<td class="segtd">分组</td>
						<td class="segtd">间隔</td>
						<td class="segtd">应用</td>
					</tr>
					<tr>
...
{
	var html = `<div class="searchbar"><input type="text" class="search" placeholder="搜索" oninput="search(this)" />
...
function toggleNl() {
	...
	if (nlA)
	{
		showToast(`计时开始。 灯会 ${nlTar > 0 ? "开":"关"} ${nlFade ? "在":"在"} ${nlDur} 分钟后.`);
	} else {
		showToast('计时器已停用。');
	}
	var obj = {"nl": {"on": nlA}};
...
function toggleSync() {
	...
	if (syncSend)
	{
		showToast('网络中的其他灯光现在将同步到此灯光。');
	} else {
		showToast('此灯和网络中的其他灯将不再同步。');
	}
...
function resetUtil() {
	var cn = `<button class="btn btn-s btn-i" onclick="makeSeg()"><i class="icons btn-icon">&#xe18a;</i>添加片段</button><br>`;
	d.getElementById('segutil').innerHTML = cn;
}
...
function makeP(i,pl) {
  ...
  <button class="btn btn-i btn-p" onclick="testPl(${i}, this)"><i class='icons btn-icon'>&#xe139;</i>Test</button>`;
	...
	return `
	<input type="text" class="ptxt noslide" id="p${i}txt" autocomplete="off" maxlength=32 value="${(i>0)?pName(i):""}" placeholder="输入名称..."/><br>
	<div class="c">快速加载标签： <input type="text" class="stxt noslide" maxlength=2 value="${qlName(i)}" id="p${i}ql" autocomplete="off"/></div>
	<div class="h">(留空为无)</div>
	<div ${pl&&i==0?"style='display:none'":""}>
	<label class="check revchkl">
    ${pl?"Show playlist editor":(i>0)?"Overwrite with state":"使用当前状态"}
  ...
  <div class="po1" id="p${i}o1">
	...
	<div class="c">保存至 ID <input class="noslide" id="p${i}id" type="number" oninput="checkUsed(${i})" max=250 min=1 value=${(i>0)?i:getLowestUnusedP()}></div>
	<div class="c">
		<button class="btn btn-i btn-p" onclick="saveP(${i},${pl})"><i class="icons btn-icon">&#xe390;</i>保存 ${(pl)?"播放列表":(i>0)?"更改":"预设"}</button>
		${(i>0)?'<button class="btn btn-i btn-p" id="p'+i+'del" onclick="delP('+i+')"><i class="icons btn-icon">&#xe037;</i>删除 '+(pl?"播放列表":"预设"):
						'<button class="btn btn-p" onclick="resetPUtil()">取消'}</button>
	</div>
...
function resetPUtil() {
	var cn = `<button class="btn btn-s btn-i" onclick="makePUtil()"><i class="icons btn-icon">&#xe18a;</i>创建预设</button><br>
            <button class="btn btn-s btn-i" onclick="makePlUtil()"><i class='icons btn-icon'>&#xe139;</i>创建播放列表</button><br>`;
...
function saveP(i,pl) {
	...
	if (!d.getElementById(`p${i}cstgl`).checked) {
		...
		try {
			...
			obj.win = raw;
			if (raw.length < 2) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; 请先输入API命令";
				return;
			} else if (raw.indexOf('{') > -1) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; 自定义JSON API命令中存在语法错误";
				return;
			} else if (raw.indexOf("Please") == 0) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; 请先刷新页面，然后再修改此预设";
				return;
			}
		}
		obj.o = true;
